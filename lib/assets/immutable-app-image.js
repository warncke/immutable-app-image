...
function showUploadImage (elm, imageType) {
    ...
    var immutableAppImage = new ImmutableAppImage({
        $wrapper: $modal.find('.immutable-app-image-upload-wrapper'),
        imageType: imageType,
    })
    // set custom success handler
    immutableAppImage.uploadSuccess = function (xhr, status, data) {
        // set id for input
        $elm.closest('.immutable-app-image-field').find('input').val(data.id)
        console.log($elm.closest('immutable-app-image-field'))
        // close upload modal
