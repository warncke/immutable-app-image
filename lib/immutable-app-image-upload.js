...
async function processPrimaryImage () {
    // set white/transaprent background for image
    this.sharp.background('white')
    ...
    if (defined(this.meta.cropData)) {
        ...
        if (left < 0) {
            ...
            // extend image
            this.sharp.extend({top: 0, bottom: 0, left: extend, right: 0})
        ...
        if (top < 0) {
            ...
            // extend image
            this.sharp.extend({top: extend, bottom: 0, left: 0, right: 0})
            // add extend value to width
        ...
        if (height + top > this.sharpMetaData.height) {
            ...
            // extend image
            this.sharp.extend({top: 0, bottom: extendHeight, left: 0, right: 0})
        ...
        if (width + left > this.sharpMetaData.width) {
            ...
            // extend image
            this.sharp.extend({top: 0, bottom: 0, left: 0, right: extendWidth})
...
async function processProfileImages () {
    return Promise.each(this.imageProfiles, imageProfile => {
        ...
        if (!imageProfile.data.pregenerate) {
            return
        }
        // get promise to be resolved when image is done processing
        var processPromise = this.processImage(this.sharpMetaData, imageProfile.data)
        // save processed image
        .then(async processedImage => {
            // wait for image record to be created
            var imageRecord = await this.imageRecordPromise
            // get file name for saving
            var saveAsFileName = this.getSaveAsFileName(this.path, this.fileName, imageProfile.data.fileType, imageRecord.id, imageProfile.data.imageProfileName)
            // save image
            await this.config.fs.writeFile(saveAsFileName, processedImage)
        })
        // add promise to list
