...
async function getUpload (args) {
    ...
    var view = {}
    // load image types
    view.imageTypes = await this.model.imageType.select.all.query()
...
async function postUpload (args) {
    ...
    var [imageType, imageProfiles] = await Promise.all([
        this.model.imageType.select.by.id(meta.imageTypeId),
    ])
    // require image type
